...
fn main() {
    ...
    loop {
        ...
        unsafe {
            if (GetAsyncKeyState(0x26) as u32 & 0x8000) != 0 {
                dp_forward = 0.1;
            }
            if (GetAsyncKeyState(0x28) as u32 & 0x8000) != 0 {
                dp_forward = -0.1;
            }

            if (GetAsyncKeyState(0x25) as u32 & 0x8000) != 0 {
                dp_sides = 0.1;
            }
            if (GetAsyncKeyState(0x27) as u32 & 0x8000) != 0 {
                dp_sides = -0.1;
            }
        }
        ...
        f_cam_x = p_cam_x + r_cam_x + dp_forward*r_cam_x;
        f_cam_z = p_cam_z + r_cam_z + dp_forward*r_cam_z;
        f_cam_y = p_cam_y + r_cam_y + dp_forward*r_cam_y;

        p_cam_x = p_cam_x + dp_forward*r_cam_x;
        p_cam_z = p_cam_z + dp_forward*r_cam_z;
        ...
        restart_mouse = !restart_mouse;
        unsafe {
            if (GetAsyncKeyState(0x13) as u32 & 0x8000) != 0 {
                active = !active;
            ...
            if (GetAsyncKeyState(0x23) as u32 & 0x8000) != 0 {
